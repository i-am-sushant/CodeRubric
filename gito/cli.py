...
# Import RAG-enhanced review (optional)
try:
    from coderubric.core import review_with_rag, RAGReviewConfig
    RAG_AVAILABLE = True
except ImportError:
    RAG_AVAILABLE = False
...
@app.callback(
    ...
    help="\bCodeRubric is an open-source AI code reviewer that works with any language model provider."
...
def cmd_review(
    ...
    rag: bool = typer.Option(
        default=False,
        help="Enable RAG (Retrieval-Augmented Generation) for context-aware code review. "
             "Requires OPENAI_API_KEY environment variable."
    ),
    rag_index_only: bool = typer.Option(
        default=False,
        help="Only index the repository for RAG, don't perform review."
    ),
):
    ...
    with get_repo_context(url, _what) as (repo, out_folder):
        ...
        # Use RAG-enhanced review if requested and available
        if rag and RAG_AVAILABLE:
            logging.info("Using RAG-enhanced code review...")
            rag_config = RAGReviewConfig(
                enabled=True,
                openai_api_key=os.getenv("OPENAI_API_KEY", ""),
                index_repo_before_review=not rag_index_only
            )
            if rag_index_only:
                from coderubric.core.rag_review import RAGCodeReviewer
                reviewer = RAGCodeReviewer(rag_config)
                index_stats = asyncio.run(reviewer.index_repository(repo))
                logging.info(f"Repository indexed successfully: {index_stats}")
                return
            asyncio.run(
                review_with_rag(
                    repo=repo,
        ...
        elif rag and not RAG_AVAILABLE:
            logging.warning("RAG requested but not available. Install with: pip install chromadb openai")
            logging.info("Falling back to standard review...")
            asyncio.run(
                review(
                    repo=repo,
                    target=review_target,
