...
@app.command(
    name="deploy",
    help="\bCreate and deploy CodeRubric workflows to your CI pipeline for automatic code reviews."
...
def deploy(
    ...
    """Deploy CodeRubric to repository's CI pipeline for automatic code reviews."""
    print(logo())
    ...
    for file_config in workflow_files.values():
        ...
        if file.exists():
            message = f"CodeRubric CI workflow already exists at {utils.file_link(file)}"
            if rewrite:
                ui.warning(message)
            else:
    ...
    for key, file_config in workflow_files.items():
        ...
        if file.exists() and "merge_function" in file_config:
            ui.warning("Merging CodeRubric CI workflow into existing file:", utils.file_link(file))
    ...
    print(
        mc.ui.green("CodeRubric CI workflows have been created.\n"),
                ...
                if git_platform_type == PlatformType.GITHUB:
                    try:
                        ...
                        res = api.pulls.create(
                            head=to_branch,
                            base=base,
                            title="Add CodeRubric CI workflows",
                        )
...
def _try_commit_workflow_changes(repo: Repo) -> bool:
    ...
    """
    try:
        repo.git.commit("-m", "Add CodeRubric CI workflows")
...
def _configure_llm(
    ...
    if not api_type:
        api_type = mc.ui.ask_choose(
            "Which language model API should CodeRubric use?",
            api_types,
        )
